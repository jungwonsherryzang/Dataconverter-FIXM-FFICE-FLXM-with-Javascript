...
// Converting XML Schema names to namespaced ones below
var BASE_TYPES = {};

BASE_TYPES.string = BASE_TYPES.normalizedString = BASE_TYPES.token = BASE_TYPES.language = BASE_TYPES.NMTOKEN 
                  = BASE_TYPES.Name = BASE_TYPES.NCName = BASE_TYPES.ID = BASE_TYPES.IDREF = BASE_TYPES.ENTITY = {
...
BASE_TYPES.integer = BASE_TYPES.nonPositiveInteger = BASE_TYPES.negativeInteger = BASE_TYPES.long = BASE_TYPES.int 
                   = BASE_TYPES.short = BASE_TYPES.byte = BASE_TYPES.nonNegativeInteger = BASE_TYPES.unsignedLong 
                   = BASE_TYPES.unsignedInt = BASE_TYPES.unsignedShort = BASE_TYPES.unsignedByte = BASE_TYPES.positiveInteger = {
  parse: function (value) {
...
BASE_TYPES.duration = BASE_TYPES.time = BASE_TYPES.gYearMonth = BASE_TYPES.gYear = BASE_TYPES.gMonthDay 
                    = BASE_TYPES.gDay = BASE_TYPES.gMonth = BASE_TYPES.QName = BASE_TYPES.NOTATION = {
  parse: function (value) {
    return value;
  }
};

//Converting XML Schema names to namespaced ones
...
XsdSchema.prototype.parseSimpleType = function (input) {
  ...
  _.each(input[self.xsPrefix + 'simpleType'] || [], function (simpleType) {
    ...
    // TODO: Support other simple types -> restriction, list, union
    //TRY: LIST
    //list
    if (simpleType[self.xsPrefix + 'list']) {
      assert(simpleType[self.xsPrefix + 'list'].length === 1, simpleType[self.xsPrefix + 'list']);

      var union = simpleType[self.xsPrefix + 'list'][0];

      type.base = _.map(list.$.itemType.split(/\s+/), function (base) {
        return self.namespacedName(base);
      });

      delete list.$.itemType;

      assert(_.isEmpty(list.$), list.$);

      delete list.$;
      
      assert(_.isEmpty(list), list);
    }
    delete simpleType[self.xsPrefix + 'list'];
...
XsdSchema.prototype.parseTypes = function (input) {
  ...
  _.each(input[self.xsPrefix + 'complexType'] || [], function (complexType) {
    ...
    if (complexType.$.mixed && BASE_TYPES['{http://www.w3.org/2001/XMLSchema}boolean'].parse(complexType.$.mixed)) {
      type.mixed = true; // Can be mixed content, both text and elements
    }
  ...
  //Ignoring annotations and top-level attributes
  ...
  _.each(input[self.xsPrefix + 'element'] || [], function (element) {
    ...
    if (element.$.ref) {
      ...
      if (element.$.type) {
        ...
        // Type is nested inside the element, so create out own name for it
        var typeName = elementName + '-type-' + randomString();

        // Then pretending that it is defined without own name
  ...
  _.each(input[self.xsPrefix + 'attribute'] || [], function (attribute) {
    if (attribute.$.ref) {
      ...
      if (attribute.$.type) {
        ...
        // Type is nested inside the attribute, so create out own name for it
        ...
        // Only simpletypes are allowed for attributes
...
// Pretending that everything is a 2001 XML Schema
...
var XsdMixin = {
  ...
  // Returns imports and includes object in a callback. 
